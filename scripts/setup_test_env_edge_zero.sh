...
echo "Setting up network rwo namespaces for EDGE_ZERO testing..."
...
echo "  sudo ip netns exec server ./build/edge_zero server  ${SERVER_TUN_IP} ${SERVER_PORT}"
echo ""
echo "To run the client (in another terminal):"
echo "  sudo ip netns exec client ./build/edge_zero client ${CLIENT_TUN_IP} ${SERVER_PORT} ${SERVER_IP}"
echo ""
echo "Create route for client (-> to make rp_filter happy, i.e. server TUN addr can be accessed via tun0):"
echo "  sudo ip netns exec client ip route add  ${SERVER_TUN_IP%.*}.0/24 dev tun0"
echo "Create route for server (-> to make reply IP from http server to find tun1)):"
echo "  sudo ip netns exec server ip route add ${CLIENT_TUN_IP%.*}.0/24 dev tun1"
echo""
