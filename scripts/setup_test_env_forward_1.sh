...
sudo ip link set veth1 netns forward
sudo ip link set veth2 netns forward
...
sudo ip netns exec server ip addr add ${SERVER_IP}/24 dev veth3
sudo ip netns exec server ip link set veth3 up
sudo ip netns exec server ip link set lo up

# Configure forward namespace
echo "Configuring forward namespace..."
sudo ip netns exec forward ip addr add ${FORWARD_IP_1}/24 dev veth1
sudo ip netns exec forward ip link set veth1 up
sudo ip netns exec forward ip addr add ${FORWARD_IP_2}/24 dev veth2
sudo ip netns exec forward ip link set veth2 up
sudo ip netns exec forward ip link set lo up
sudo ip netns exec forward route add -host ${CLIENT_IP} dev veth1
sudo ip netns exec forward route add -host ${SERVER_IP} dev veth2
...
echo "  sudo ip netns exec server ./build/edge server  ${SERVER_TUN_IP} ${SERVER_PORT} certs/server_cert.pem certs/server_key.pem"
echo ""
...
echo "  sudo ip netns exec client ./build/edge client ${CLIENT_TUN_IP} ${FORWARD_IN_PORT} ${FORWARD_IP_1}"
echo ""
