...
int tun_udp_run(iouring_ctx_t *uring_ctx, connection_t *conn, int tun_fd, volatile int *running) {
    ...
    io_op_t *op = NULL;
    while (*running) {
        op = wait_for_recv(uring_ctx, running);
        if (op) {
            switch (op->op_type) {
...
int run_zero_tun2udp(iouring_ctx_t *tun2udp_ctx, int tun_fd, connection_t *conn, volatile int *running) {
    int ret = 0;

    log_info("Starting tun2udp loop (tun_fd %d)", tun_fd);

    // Main event loop
    io_op_t *op = NULL;
    while (*running) {
         // wait for a cumpleted TUN read
        op = wait_for_recv(tun2udp_ctx, running);
        if (op) {
            assert(op->op_type == OP_TYPE_TUN_READ);
            ret = tun_to_udp(conn, op, tun2udp_ctx);
            if (ret < 0)
                break;
             io_op_free(tun2udp_ctx, &op);
        } else {
            ret = -1;
            break;
        }
...
int run_zero_udp2tun(iouring_ctx_t *udp2tun_ctx, connection_t *conn, int tun_fd, volatile int *running) {
    int ret = 0;
    ...
    io_op_t *op = NULL;
    while (*running) {
         // wait for a cumpleted UDP recvmsg
        op = wait_for_recv(udp2tun_ctx, running);
        if (op) {
            assert(op->op_type == OP_TYPE_UDP_RECV);
            if (conn->addr_len == 0) {
                assert(op->addr_len > 0);
